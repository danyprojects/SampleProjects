...
namespace Bacterio.Common
{
    public class TimerController
    {
        ...
        {
            public readonly TimerController _controller = null;

            public Dispatcher(int currentTimeMs)
            {
                _controller = new TimerController(currentTimeMs);
            }

            public void Dispatch(int currentTimeMs)
            {
                _controller.Dispatch(currentTimeMs);
            }
        ...
        {
            private (int Element, int Priority)[] _nodes;
            ...
            public Queue(int currentTimeMs)
            {
            ...
            public void Push(int duration, Action callback)
            {
                int index = _freeNodeStack.GetFreeIndex();

                _timerCallbacks[index] = callback;
                _timerQueue.Enqueue(duration + _currentTimeMs, index);
            }

            public void Dispatch(int currentTimeMs)
            {
                _currentTimeMs = currentTimeMs;
                while (_freeNodeStack.Used > 0 && _timerQueue.First <= _currentTimeMs)
                {
            ...
            public void Clear()
            {
                for (int i = 0; i < _timerCallbacks.Length; i++)
                    _timerCallbacks[i] = null;
        ...
        private TimerController(int currentTimeMs)
        {
            _queue = new Queue(currentTimeMs);
        }

        /// <summary>
        /// Queues a callback to be invoked after "duration" seconds
        /// </summary>
        public void Add(double duration, Action callback)
        {
            _queue.Push((int)(duration * 1000), callback);
        }

        /// <summary>
        /// Queues a callback to be invoked after "duration" millseconds
        /// </summary>
        public void Add(int duration, Action callback)
        {
            _queue.Push(duration, callback);
        }

        private void Dispatch(int currentTimeMs)
        {
            _queue.Dispatch(currentTimeMs);
        }
        ...
        private class TimerPriorityQueue
        {
            ...
            public void Enqueue(int priority, int timerIndex)
            {
                _numNodes++;

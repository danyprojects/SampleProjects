...
namespace RO.Media
{
    public class ShieldAnimator : MonoBehaviour
    {
        ...
        public void UpdateRenderer()
        {
            ...
            Vector2 shieldAttachPoint = CurrentAction.Frames[PlayerAnimatorData.CurrentBodyFrame].attachPoint;
            Vector2 pos;
            if (shieldAttachPoint.x == 0 && shieldAttachPoint.y == 0)
                pos = new Vector2(frameData.PositionOffset[0].x, -frameData.PositionOffset[0].y);
            else
                pos = new Vector2(PlayerAnimatorData.BodyAttachPoint.x - shieldAttachPoint.x + frameData.PositionOffset[0].x,
                                 -PlayerAnimatorData.BodyAttachPoint.y + shieldAttachPoint.y - frameData.PositionOffset[0].y);
            ...
            // update shield transform            
            //Shield to change order depending on the direction since we are not supposed to see them overlap when player is showing their back
            float zOffset = (PlayerAnimatorData.Direction.BodyCamera >= 2 && PlayerAnimatorData.Direction.BodyCamera <= 5) ? 0.05f : _act.OrderInLayer;
            gameObject.transform.localPosition = new Vector3(pos.x, pos.y, zOffset);

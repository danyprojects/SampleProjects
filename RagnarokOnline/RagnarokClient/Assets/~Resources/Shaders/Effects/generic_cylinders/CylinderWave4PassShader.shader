...
Shader "Custom/Cylinder Wave 4 Pass Shader"
{
	Properties
	{
		...
		[PerRendererData] _Tint2("Tint 2", Color) = (1,1,1,1)
		[PerRendererData] _Tint3("Tint 3", Color) = (1,1,1,1)
		[PerRendererData] _Tint4("Tint 4", Color) = (1,1,1,1)
	...
	SubShader
	{
		...
		Pass //PASS 1
		{
			...
			v2p vert(IN input)
			{
				...
				float isBot = 1 - isTop;
				float elapsed = _Time.y - _StartTime[0];
				//y equation is for going from min to max height, at speed height speed
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[0] * isTop + _BottomWidth[0] * isBot, 0, 0, 0,
					0, _MinHeight[0] + (_MaxHeight[0] - _MinHeight[0]) * (1 + cos(pi + elapsed * _HeightSpeed[0])) / 2, 0, 0,
		...
		Pass //PASS 2
		{
			...
			v2p vert(IN input)
			{
				...
				float elapsed = _Time.y - _StartTime[0];
				//y equation is for going from min to max height, at speed height speed
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[1] * isTop + _BottomWidth[1] * isBot, 0, 0, 0,
					0, _MinHeight[1] + (_MaxHeight[1] - _MinHeight[1]) * (1 + cos(pi + elapsed * _HeightSpeed[1])) / 2, 0, 0,
					0, 0, 1 + _TopWidth[1] * isTop + _BottomWidth[1] * isBot, 0,
					0, 0, 0, 1);
			...
			half4 frag(v2p o) : COLOR
			{
				float4 mainTexColor = tex2D(_MainTex, o.uv0);
				float elapsed = clamp(_Time.y - (_StartTime[0] + _StartTime[3] - _StartTime[1]), 0, 32672);
				_Tint2.a = clamp(_Tint2.a - (elapsed / (_StartTime[1] + 0.001)) * _Tint2.a, 0, 1);
				return mainTexColor * _Tint2;
			}
			ENDCG
		}
		...
		Pass //PASS 3 
		{
			...
			v2p vert(IN input)
			{
				...
				float elapsed = _Time.y - _StartTime[0];
				//y equation is for going from min to max height, at speed height speed
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[2] * isTop + _BottomWidth[2] * isBot, 0, 0, 0,
					0, _MinHeight[2] + (_MaxHeight[2] - _MinHeight[2]) * (1 + cos(pi + elapsed * _HeightSpeed[2])) / 2, 0, 0,
					0, 0, 1 + _TopWidth[2] * isTop + _BottomWidth[2] * isBot, 0,
					0, 0, 0, 1);
			...
			half4 frag(v2p o) : COLOR
			{
				float4 mainTexColor = tex2D(_MainTex, o.uv0);
				float elapsed = clamp(_Time.y - (_StartTime[0] + _StartTime[3] - _StartTime[1]), 0, 32672);
				_Tint3.a = clamp(_Tint3.a - (elapsed / (_StartTime[1] + 0.001)) * _Tint3.a, 0, 1);
				return mainTexColor * _Tint3;
			}
			ENDCG
		}
		...
		Pass //PASS 4
		{
			...
			v2p vert(IN input)
			{
				...
				float elapsed = _Time.y - _StartTime[0];
				//y equation is for going from min to max height, at speed height speed
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[3] * isTop + _BottomWidth[3] * isBot, 0, 0, 0,
					0, _MinHeight[3] + (_MaxHeight[3] - _MinHeight[3]) * (1 + cos(pi + elapsed * _HeightSpeed[3])) / 2, 0, 0,
					0, 0, 1 + _TopWidth[3] * isTop + _BottomWidth[3] * isBot, 0,
					0, 0, 0, 1);
			...
			half4 frag(v2p o) : COLOR
			{
				float4 mainTexColor = tex2D(_MainTex, o.uv0);
				float elapsed = clamp(_Time.y - (_StartTime[0] + _StartTime[3] - _StartTime[1]), 0, 32672);
				_Tint4.a = clamp(_Tint4.a - (elapsed / (_StartTime[1] + 0.001)) * _Tint4.a, 0, 1);
				return mainTexColor * _Tint4;
			}
			ENDCG
		}

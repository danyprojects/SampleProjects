...
Shader "Custom/Cylinder 4 Pass Shader"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass //PASS 2
		{
			...
			v2p vert(IN input)
			{
				...
				//rotation for Y axis
				float c2 = cos(_Time.y * _RotateSpeed[1]);
				float s2 = sin(_Time.y * _RotateSpeed[1]);
				float4x4 rotate = float4x4(c2, 0, s2, 0,
											0, 1, 0, 0,
										  -s2, 0, c2, 0,
											0, 0, 0, 1);
				...
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[1] * isTop + _BottomWidth[1] * isBot, 0, 0, 0,
					0, 1 * clamp(_MinHeight[1] + (_Time.y - _StartTime[0]) * _HeightSpeed[0], 0, _MaxHeight[1]), 0, 0,
		...
		Pass //PASS 3
		{
			...
			v2p vert(IN input)
			{
				...
				//rotation for Y axis
				float c2 = cos(_Time.y * _RotateSpeed[2]);
				float s2 = sin(_Time.y * _RotateSpeed[2]);
				float4x4 rotate = float4x4(c2, 0, s2, 0,
					0, 1, 0, 0,
					-s2, 0, c2, 0,
					0, 0, 0, 1);
				...
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[2] * isTop + _BottomWidth[2] * isBot, 0, 0, 0,
					0, 1 * clamp(_MinHeight[2] + (_Time.y - _StartTime[0]) * _HeightSpeed[2], 0, _MaxHeight[2]), 0, 0,
		...
		Pass //PASS 4
		{
			...
			v2p vert(IN input)
			{
				...
				float4x4 scaleMatrix = float4x4(1 + _TopWidth[3] * isTop + _BottomWidth[3] * isBot, 0, 0, 0,
					0, 1 * clamp(_MinHeight[3] + (_Time.y - _StartTime[0]) * _HeightSpeed[3], 0, _MaxHeight[3]), 0, 0,
